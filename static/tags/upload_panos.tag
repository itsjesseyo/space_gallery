...
<div class="divider"/>
 ...
 -->							</div>
				...
				<form onsubmit={ create_new_slide } action='/upload_pano' method=post enctype=multipart/form-data >
...
<script>
		...
		this.can_edit = false
		this.get_data = fetchival(window.location.origin+'/api/v1/get/panos')
		this.post_data = fetchival(window.location.origin+'/api/v1/update/panos')
		this.delete_data = fetchival(window.location.origin+'/api/v1/delete/panos')
